...
def start_interaction(model_id, objective, project_name):
	"""Start an interactive session with a language model to navigate and interact with web pages.
	
	This function initializes a crawler and enters an interaction loop where it communicates with a language model (LLM) to
	perform actions on a web page. The LLM provides commands based on the current state of the page, which are then executed
	by the crawler. The session runs for up to five iterations or until interrupted by the user.
	
	Args:
	    model_id (str): The identifier of the language model to use.
	    objective (str): The goal or task the language model should focus on during the interaction.
	    project_name (str): The name of the project, used for naming screenshots.
	"""
	_crawler = Crawler()

	def print_help():
		"""Print a help message listing available commands.
		
		This function displays a formatted string containing various commands and their corresponding actions. The commands
		include options to visit URLs, scroll up or down, click on elements, type text, view commands, run suggested commands,
		and change the objective.
		"""
		print(
			"(g) to visit url\n(u) scroll up\n(d) scroll down\n(c) to click\n(t) to type\n" +
	...
	def get_gpt_command(objective, url, previous_command, browser_content):
		"""Generates a GPT command based on given objective, URL, previous command, and browser content.
		
		This function constructs a prompt using a template and replaces placeholders with the provided inputs. It then sends
		this prompt to a language model (LLM) for inference and returns the generated response.
		
		Args:
		    objective (str): The primary goal or task to be achieved.
		    url (str): The URL of the webpage being interacted with, truncated to 100 characters.
		    previous_command (str): The last command issued, used as context.
		    browser_content (str): The content of the current webpage, truncated to 4500 characters.
	...
	def run_cmd(cmd):
		"""Execute a command string to interact with a crawler.
		
		This function takes a command string and performs corresponding actions such as scrolling, clicking, typing, or
		submitting text. The function processes the command based on its prefix and extracts necessary parameters from the
		command string.
		
		Args:
		    cmd (str): A command string indicating the action to perform.
		        Supported commands are "SCROLL UP", "SCROLL DOWN", "CLICK", "TYPE",
		        and "TYPESUBMIT".
		"""
		cmd = cmd.split("\n")[0]
